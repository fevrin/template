...
.PHONY: save-repo-configs
API_ENDPOINT := '/repos/fevrin/template'
save-repo-configs: ## Repository: save all relevant repository configs
   -@echo
   -@echo '=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-='
   -@echo $(shell echo '$@' | tr '[:lower:]' '[:upper:]')
   -@echo '=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-='
   ...
   @CLICOLOR_FORCE= \
    ...
    gh api \
      ...
      /repos/fevrin/template \
      --jq 'with_entries(if(.key|test("(allow|delete|has|is)_.+")) then ({key: .key, value: .value }) else empty end)' |\
      jq '.' \
   >.github/configs/repo_general.json

   @for ruleset in \
      $$(gh api \
         -H "Accept: application/vnd.github+json" \
         -H "X-GitHub-Api-Version: 2022-11-28" \
         $(API_ENDPOINT)/rulesets \
         --jq '.[].id'); do \
      CLICOLOR_FORCE= \
      NO_COLOR=true \
      gh api \
      -H "Accept: application/vnd.github+json" \
      -H "X-GitHub-Api-Version: 2022-11-28" \
      $(API_ENDPOINT)/rulesets/$${ruleset} \
      --jq 'with_entries(select(.key|test("^(id|source|node_id|.+_at|_links)$$") | not))'; \
    done |\
    jq '.' \
    > .github/configs/repo_rulesets.json
